...
function FeaturedMovie ({title, overlayStyle, featuredMovie, videoId, setTruncLine, truncLine}) {
	...
	return(
		<div className="app__featured">
			...
			<h2 className="app__featuredTitle">{featuredMovie.title || featuredMovie.original_title || featuredMovie.name || featuredMovie.original_name}<span className="app__featuredYear">({getReleaseYear(movie.release_date || movie.first_air_date)})</span></h2>
			<TextTruncate
				line={truncLine}
				element="p"
